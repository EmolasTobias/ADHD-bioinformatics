...
dds2 = DESeqDataSetFromTximport(txi, colData=metadata, design=~treatment)

# Relevel the treatment factor so that "WT" is the reference level
dds2$treatment = relevel(dds2$treatment, ref="WT")

# Run the test. Here you can choose between LRT or Wald test
dds2 = DESeq(dds2, test="Wald")

# Retrieve the results
res = results(dds2, parallel=TRUE, alpha=0.05)


# Create a dataset that has all the genes from the sequencing
write.table(res, file=paste(results_dir, '/Wald_NoThreshold_WT_vs_MUTANT.tsv', sep=''), quote=FALSE, sep='\t')
...
write.table(sorted_filtered_results, file=paste(results_dir, '/Wald_NoThreshold_WT_vs_MUTANT_filtered.tsv', sep=''), quote=FALSE, sep='\t')
write.table(anti_results, file=paste(results_dir, '/Wald_NoThreshold_WT_vs_MUTANT_anti.tsv', sep=''), quote=FALSE, sep='\t')
...
# a simple PCA
plotPCA(rlog(dds2), intgroup=c('treatment')) + ggtitle('effect WT vs MUTANT')
...
dds2_df <- as.data.frame(res)  # Convert DESeq2 results to dataframe
dds2_df$ensembl_gene_id <- rownames(dds2_df)  # Make row names a column
...
# Save the results from Desec2 with no filters and Wald test instead of LRT

write.table(dds2_annotated, file=paste(results_dir, '/Wald_NoThreshold_WT_vs_MUTANT_annotated.tsv', sep=''), quote=FALSE, sep='\t')

library(dplyr)

# Define the genes of interest
genes_of_interest <- c("tnfa", "il1b", "il6", "il10", "aif1l", "rpl13a")

# Filter dataset
dds2_filtered <- dds2_annotated %>%
...
final_table <- dds2_filtered %>%
  select(external_gene_name, baseMean, log2FoldChange, padj) %>%
...
final_table <- dds2_filtered[, c("external_gene_name", "baseMean", "log2FoldChange", "padj")]
colnames(final_table) <- c("Gene", "Mean_Expression", "Log2FC", "Adjusted_p")

write.csv(final_table, "Microgligal_final_table.csv", row.names = FALSE)


# Ensure no zero padj values (to avoid log10 issues)
dds2_annotated$padj[dds2_annotated$padj == 0] <- min(dds2_annotated$padj[dds2_annotated$padj > 0], na.rm = TRUE)

# Define plotting variables
dds2_annotated$logP <- -log10(dds2_annotated$padj)
...
dds2_annotated$significance <- "Not Significant"
dds2_annotated$significance[dds2_annotated$padj < padj_threshold & dds2_annotated$log2FoldChange > logFC_threshold] <- "Upregulated"
dds2_annotated$significance[dds2_annotated$padj < padj_threshold & dds2_annotated$log2FoldChange < -logFC_threshold] <- "Downregulated"

# Identify the top 20 most significant genes
top15 <- dds2_annotated[order(-dds2_annotated$logP), ][1:15, ]

# Save the new annotaded as a file
dir.create(results_dir)
write.table(dds2_annotated, file=paste(results_dir, '/NEW_WT_vs_MUTANT_annotated_update.tsv', sep=''), quote=FALSE, sep='\t')
...
ggplot(dds2_annotated, aes(x = log2FoldChange, y = logP, color = significance)) +
  ...
  geom_text_repel(data = top15, aes(label = external_gene_name), 
...
# Categorize genes
dds2_annotated$Significance <- "Not Significant"
dds2_annotated$Significance[dds2_annotated2$padj < padj_threshold & dds2_annotated2$log2FoldChange > logFC_threshold] <- "Upregulated"
dds2_annotated$Significance[dds2_annotated2$padj < padj_threshold & dds2_annotated2$log2FoldChange < -logFC_threshold] <- "Downregulated"
...
# Define the genes of interest
genes_of_inflammatory_interest <- c("il6st", "il6r", "aif1l", "tnfrsf1a", "tnfrsf11b", "il10rb", "il1rapl2", "il15l", "ifngr1l", "cxcl12b", "cxcl12a", "icam3")

# Filter dataset
CNS_inflammation <- dds2_annotated %>% 
